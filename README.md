...
- [2. Context 传递登录态的技术方案](#2-context-传递登录态的技术方案)
- [3. JWT 签名（signToken）与验签（VerifyToken）原理](#3-jwt-签名signtoken与验签verifytoken原理)
- [4. MCP 协议的两种传输方式（SSE Transport / Streamable HTTP）](#4-mcp-协议的两种传输方式sse-transport--streamable-http)
- [5. Nginx proxy_pass 末尾斜杠的差别（反代路径改写规则）](#5-nginx-proxy_pass-末尾斜杠的差别反代路径改写规则)
- [6. 多后端服务共存时的 URL 路径分层与网关前缀最佳实践](#6-多后端服务共存时的-url-路径分层与网关前缀最佳实践)
- [7. 反代到后端为什么不用 HTTP/2？](#7-反代到后端为什么不用-http2)
- [8. 仅 loopback 的 gRPC 服务与网关反代的意义及实现](#8-仅-loopback-的-grpc-服务与网关反代的意义及实现)
- [9. 前端调用 gRPC 服务还是 HTTP 网关？两者有何区别？](#9-前端调用-grpc-服务还是-http-网关两者有何区别)
- [10. grpc-gateway 路由注册机制与双端口架构](#10-grpc-gateway-路由注册机制与双端口架构)
- [11. 契约与部署拓扑解耦与资源导向的 API 路径设计](#11-契约与部署拓扑解耦与资源导向的-api-路径设计)
- [12. Go 编译期接口实现断言（`var _ Iface = (*T)(nil)`）](#12-Go-编译期接口实现断言var-_-iface--tnil)

### 服务器应用部署
- [1. 在服务器上编译基于 go 实现的后端代码并部署的流程](#1-服务器上编译基于-go-实现的后端代码并部署的流程)
- [2. 在服务器上编译基于 vue 实现的前端代码并部署的流程](#2-服务器上编译基于-vue-实现的前端代码并部署的流程)
...
- [2. 为什么 psql 在 /usr/ 下，而 mysql 在 /usr/local/ 下（FHS 与安装方式）](#2-为什么-psql-在-usr-下而-mysql-在-usrlocal-下fhs-与安装方式)
...
## 3. JWT 签名（signToken）与验签（VerifyToken）原理
#### 一、JWT 是什么
...
## 4. MCP 协议的两种传输方式（SSE Transport / Streamable HTTP）
#### 1. SSE Transport（/sse）
...
## 5. Nginx proxy_pass 末尾斜杠的差别（反代路径改写规则）
### 问题
...
## 8. 仅 loopback 的 gRPC 服务与网关反代的意义及实现
### 问题
...
## 9. 前端调用 gRPC 服务还是 HTTP 网关？两者有何区别？
### 问题
...
## 12. Go 编译期接口实现断言（`var _ Iface = (*T)(nil)`）
### 问题
